...
Properties-Datei angegeben werden, in der bestimmte Werte für die Attribute der \gks enthält.\\
...
Anforderung \hyperref[AS4]{AS4}, wird die erste Variante becorzugt (siehe
...
\begin{tabular}[c]{|l|l|}
  		  ...
  		  \\
				...
				\hline
			\end{tabular} 
 \caption{Basiskomponenten mit spzifischen Metadaten}
 \label{tab_comp}
\end{table}
\noindent
Mit Außnahme der Metadaten des TabPanes sind alle Angaben zu anderen
...
dafür ist vor allem, dass die Anforderung \hyperref[AA5]{AA5} und
\hyperref[AS4]{AS4} mehr Beachtung
finden soll. Somit werden die bekannten \Ks wie folgt definiert (siehe Listing \ref{LST_Bt+Lb3}).
\begin{lstlisting}[{caption = 3.
Iteration:
...
werden (siehe Listing \ref{LST_Tab3}). In den folgenden Beispielen wird davon
