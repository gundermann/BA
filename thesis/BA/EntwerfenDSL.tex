...
\section{3. Iteration}
\subsection*{Analyse der Metadaten des GUIs}
Um eine Referenzierung von UI-Komponenten und dem entsprechenden Layout aus der
...
UI-Komponenten unterscheiden zu müssen, wird innerhalb der GUI-Beschreibung
ein optionales Feld benötigt, mittels dessen unterschiedlichen UI-Komponenten
...
UI-Komponenten zu den Bereichen entfällt ebenfalls. Die Strukturierung wird über
eine Aufzählung der UI-Komponenten vorgenommen.\\
...
UI-Komponenten (\emph{Element}) in einer Liste abgelegt wird) und Layout
...
einelnen Klassen. Grund dafür ist, dass die speziellen Properties dieser \Ks
obligatorisch sind. Die folgenden Abbildungen zeigen diese drei \Ks
...
bildet die Referenz zu den in die TabView einzubindenen UI-Komponenten.
...
\begin{lstlisting}[{caption = 3.
Iteration:
Eingebundene UI-Komponenten},{label = LST_Use3}] 
...
\end{lstlisting}
Die \Ks TabView werden die verwendeten UI-Komponenten der einzelnen Tabs
...
welche UI-Komponenten in welcher GUI-Beschreibung verändert werden soll.
...
Angabe ähnelt der Zuweisung von UI-Komponenten zu Bereichen, wie es auch der
ersten und zweiten Iteration bekannt ist. Da dieses Konstrukt vereinfacht werden
sollte, werden die UI-Komponenten in der richtigen Reihenfolgen nach dem
