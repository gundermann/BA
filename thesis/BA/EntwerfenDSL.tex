...
\emph{AreaCount}).\\
Für die Beschreibung der Layoutkomponenten wird zwischen zwei Typen
...
Area:4<-"Multiselect"
\end{lstlisting}
Mit diesem \emph{GUI-Skript} soll eine \gui wie in Abbildung \ref{guiKS1}
dargestellt werden.
\myBigFigure{}{}{guiKS1}
\noindent
Die Beschreibung der \gui beginnt mit der Deklaration der Anzahl der verwendeten
Bereiche. Die Bezeichnung \emph{Area} wurde bewusst gewählt, da dieser Begriff
...
abgebildet. Der Begriff wurde gewählt, weil eine \gui, die als
...
An die Definition des Layouttyps schließt sich die Angabe der eingebundenen
...
trägt.\\
Anschließend werden die einzelnen Komponentendefinitionen durch das
 Schlüsselwort \emph{DEF} eingeleitet und durch das Schlüsselwort \emph{END DEF}
...
\emph{Typ} (\emph{Button}) der Komponentendefinition zu finden. Weiterhin
wurde eine Interaktion für den \emph{Button} definiert, deren Aktion ausgelöst
wird, wenn er betätigt wird (\emph{CLICK}). In diesem Fall wird die Aufschrift
des \emph{Buttons} in \glqq Du hast interagiert\grqq geändert.\\
