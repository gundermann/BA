...
// START PROCESSING
    ...
    function placeBannerSummaryBioAndConfDesc() {
        if (inHorizontal) {
            ...
            $leftBottomTextCont.append($summary);
            $leftPane.append('<div id="id_300x250_banner_top"><script type="text/javascript">googletag.cmd.push(function() { googletag.display("id_300x250_banner_top"); });</script></div>');

            // append in 'text_height_fix_box' - box with fix height
            $rightBottomTextCont.append($authorBio).append($confDesc);
            $rightPane.append($slidesContainer).append($('#top > .rightSide #text_height_fix_box'));
        } else {
                // move the summary and conference texts in the right
            $rightTopTextCont.append($summary).append($confDesc);
            ...
            if (!inFullScreen) {
                ...
                $bottomPane.append('<div id="id_300x250_banner_top"><script type="text/javascript">googletag.cmd.push(function() { googletag.display("id_300x250_banner_top"); });</script></div>');
            }

            // last biolabel and biotext
            $bottomPane.append($authorBio);
...
$(function () {    
    $('#mp3').click(function () {
        if (!loggedIn) {
            ...
            PresentationDownloads.loadMp3.call();
        }
    });
    $('#slides').click(function () {
        if (!loggedIn) {
            ...
            PresentationDownloads.loadPdf.call()();
        }
    });
});
