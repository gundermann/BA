...
weil dafür entsprechende Informationen fehlen. Dennoch kann der Klassenrumpf
...
Der Quellcode für den Baum wird ähnlich generiert. Wenn der Input-Typ des
...
(siehe \hyperref[cd]{Anlage}) im Projekt \emph{Explorer}.
...
'''
\end{lstlisting}
Die generierten Dateien sind auf dem beiliegenden Datenträger (siehe
\hyperref[cd]{Anlage}) zu finden.
...
\ref{explorergui}) zeigt die \gui, welche durch die Klasse
...
wird, ist diese Aufgabe entsprechend einfach. Dafür werden lediglich eine
Oberklasse und entsprechende Importe benötigt, wie Listing \ref{LST_FPGen} zu entnehmen
ist.
...
Generierung des Klassenkopfs und entsprechend benötigten Imports (siehe
...
Bei der Bestimmung der Interaktionsformen werden die im GUI-Skript
...
\end{lstlisting}
Für die Standard-Interaktionsformen muss unterschieden werden um welche
\gk es sich handelt.\\
...
werden.
\begin{lstlisting}[{caption= Generierung der Standard-Interaktionsformen von
...
Um Compilierungsfehler zu vermeiden, müssen die Methoden, die vom generierten
 Quellcode verwendet werden, ebenfalls generiert werden. Dafür ist die Methode
...
\end{lstlisting}
\parskip 20pt
Die Generator-Klasse ist auf dem beiliegenden Datenträger zu finden (siehe
\hyperref[cd]{Anlage}).
