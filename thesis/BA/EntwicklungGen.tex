...
\emph{Java-Klassen} zu erzeugen, die in das \MCF integriert werden können. In
Abbildung \ref{Abb_generator} ist dieses Konzept schematisch dargestellt. Für
...
\myBigFigure{generator.jpg}{Generierung der Klassen für das
...
Funktionalitäten der Interaktion können vom \emph{Generator} nicht
...
beschrieben. Die \gui in Abbildung \ref{Abb_antragsexpl} (siehe Anhang \ref{explorergui})
zeigt den Aufbau des \emph{Antragsexplorers}, wie sind in \pcs häufig
verwendet wird. Diese \gui wird innerhalb von \pcs zur Visualisierung und
Bearbeitung einer \emph{Antragsmappe} für einen \glossLink{Förderantrag}{Förderantrag}
verwendet.
...
der Abbildung \ref{Abb_antragsexpl} (siehe Anhang \ref{explorergui}) ist
...
\glossLink{Förderantrag}{Förderantrag} unterschiedlich sein. Zudem
werden im \emph{Web-Client} innerhalb dieses Bereichs auch die \guis
der ausgewählten Dokumente der \emph{Antragsmappe} - die Auswahl dieser ist im
\gloss{Inhaltsbaum} möglich - eingebunden. Diese Eigenschaft rechtfertigt die
...
\begin{lstlisting}[{caption=GUI-Skript für die
...
\end{lstlisting}
\noindent
Die Methoden, die zur Generierung von \gks für die \emph{GUI-Klassen} verwendet
wurden, enthalten Codeabschnitte, welche das Layout für den Prototyp
...
dem beiliegenden Datenträger (siehe
