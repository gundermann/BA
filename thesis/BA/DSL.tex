...
\subsection*{Domain Specific Language (DSL)}
  ...
  Das Entwickeln einer DSL ermöglicht es, die Abstraktion der  Sprache der Domäne anzupassen 
  ...
  technischen DSLs und fachlichen DSL statt. Markus Völter et al. unterscheiden
  diese beiden Kategorien im Allgemeinen dahingehend, dass technische DSLs von
  Programmierern genutzt werden und fachliche DSL von Domänenexperten (z. B. Kunden) \vcite{voelter}{26}.
...
Eigenschaft des Domänenfokus als ein Schlüsselelement der Definition einer DSL 
\vcite{fowler}{27f}.
...
\subsubsection*{Ausdruckskraft}
 Laut Ghosh sollten DSLs so umgesetzt werden, dass sie präzise sind. Diese
 Präzision bedingt, dass eine DSL einfach zu verstehen ist. Bei der Verwendung
 einer DSL sollte demnach der von  Dan Roam beschriebenen Prozess des visuellen
...
\subsubsection*{Höhere Qualität}
  Bei der Entwicklung einer DSL werden die Sprachkonstrukte und
  Freiheitsgrade der Sprache festgelegt. Richtig konzipiert, schränken sie den Entwickler im
  Umgang mit dieser DSL so ein, dass die Möglichkeit redundanten Quellcode zu
  ...
  wird durch die starke Abstraktion einer DSL die Wiederverwendung gefördert,
...
\subsubsection*{Bessere Kommunikation mit
  ...
  Verwendung einer DSL als reine Kommunikationplattform als vorteilhaft 
...
\subsubsection*{Großes Know-How gefordert}
  ...
  eine DSL bietet, können nur genutzt werden, wenn die DSL ausreichend gut
...
\subsubsection*{Kosten für die Entwicklung der
  ...
  Weiterhin sind in der Phase, in der die DSL entwickelt wird, keine große
...
\subsubsection*{Kakophonie}
  ...
  Das wiederum erfordert, dass die Entwickler öfter umdenken müssen, als wenn
  sie fortwährend mit einer Sprache arbeiten würden. Dieses macht den
...
\subsubsection*{Ghetto-Sprache}
  ...
  sie höchstwahrscheinlich nicht einmal diese DSLs und ihren Zweck kennen
  werden 
  \cite{fowler}{38}.\\
...
\subsubsection*{Kulturelle Herausforderungen}
  Die genannten Nachteile des Einsatzes von DSLs führen zu Äußerungen wie
  \emph{Die Entwicklung von Sprachen ist kompliziert}, \emph{Domänenexperten
  sind keine Programmierer} oder \emph{Ich möchte nicht schon wieder eine
  ...
  eingeführt werden soll \cite{voelter}{45}. Die Mitarbeiten müssen demnach
...
\subsubsection*{Unvollständige DSLs}
  ...
  durchgeführt werden, bevor mit der Entwicklung begonnen wird. Durch die
  einfache Entwicklung scheint es weniger aufwendig eine neue DSL zu schaffen,
  als nach bestehenden Ansätzen für das gleiche Problem zu suchen 
  ...
  untereinander nicht kompatibel sind. Außerdem führt der Fakt, dass die
  Entwicklung einer DSL zum Verstehen der Domäne sehr hilfreich ist, dazu, dass
  eine DSL nur aus diesem Grund entwickelt wird 
  \vcite{fowler}{38}. Das wiederum hat zur Folge, dass mehrere halb-fertige
...
ist. Sie übernehmen dabei das Typsystem der GPL \vcite{voelter}{50}. In
...
übernommen, was bei einer DSL nicht der Fall ist \vcite{voelter}{30}. Weiterhin
können DSLs so konstruiert werden, dass durch Restriktionen und Limitierungen
nur korrekte Programme geschrieben werden können. Markus Völter et al. bezeichnen diese Eigenschaft als \emph{correct-by-construction} 
\vcite{voelter}{30}.
...
werden können \vcite{fowler}{68}. Eine solche Verkettung von Methoden wird
...
i. d. R. das Verhalten von Objekten. Beim Laden des Programms werden meist
Validierungs-Annotationen verwendet. Solche Annotationen werden bspw. verwendet, um Mapping für Datenbanken
...
\begin{itemize}
  ...
  \vcite{ghosh}{218}
  \item Möglichkeit des Generierens eines Parser in unterschiedlichen Sprachen
...
der Grammatik \vcite{fowler}{245}. Die Funktionen geben dabei einen
Boolean-Wert zurück, der Auskunft darüber gibt, ob die Symbole aus dem
DSL-Skript mit den Symbolen übereinstimmen, die laut Grammatik erwartet werden
...
\vcite{fowler}{249}. Ein Nachteil ist, dass keine Grammatik definiert wird. Laut Fowler et. al. wird dadurch einer DSL ein gravierender
...
Input-Streams von einem Parser erfolgreich oder fehlerhaft verarbeitet wurde,
kann der Rest des Input-Streams an einen anderen Parser übergeben werden. Somit ist es möglich
...
Zeile Quellcode schreiben zu müssen. Im Hintergrund generiert dieser Editor den
Quellcode, welcher Sicht auf die Domäne modelliert \vcite{ghosh}{19f}.
