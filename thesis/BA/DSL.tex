...
\subsection*{Grammatik}
  \emph{Grammatiken} und insbesondere Grammatikregeln werden zur Beschreibung
  von Sprachen verwendet \vcite{hedtstueck}{23f}. Für die Definition einer \emph{Grammatik}
  wird auf den Praxisbericht \cite{gundermann5} verwiesen. \emph{Grammatiken}
...
darin, dass eine \emph{DSL} neben einem abstrahierten Vokabular auch eine spezifische \emph{Grammatik}
...
ist, müssen zwei Artefakte definiert werden. Zuerst muss eine \emph{Grammatik} in der
...
(\emph{Grammatik} und Aktionen) neu definiert werden müssen \vcite{fowler}{269}. Weiterhin arbeiten die
...
\begin{itemize}
  ...
  \item Validierung der \emph{Grammatik} durch Fehlererkennung und -behandlung
  \vcite{fowler}{272}
\end{itemize}
...
nonterminalen Symbole der \emph{Grammatik} \vcite{fowler}{245}. Die Funktionen geben dabei einen
Boolean-Wert zurück, der Auskunft darüber gibt, ob die Symbole aus dem
\emph{DSL-Skript} mit den laut \emph{Grammatik} zu erwartenden Symbolen übereinstimmen
...
sich um eine einfache \emph{Grammatik} handelt - ein großer Vorteil dieser Technik
\vcite{fowler}{249}. Ein Nachteil ist, dass keine \emph{Grammatik} definiert wird.
...
Bei der Kombination von \emph{Parsern} wird die \emph{Grammatik} mittels einer Struktur
...
auch die Lesbarkeit der \emph{Grammatik} gefördert, was bei einem \emph{RD-Parser} ein großer
