...
\begin{itemize}
  ...
  einschließig der Null). \vcite{hedtstueck}{6} \dcite{Eine endliche Folge ($x_1, \ldots, x_k$) mit $x_i
  ...
  einer Hierachrie dargestellt werden (\emph{Chomsky-Hierarchie}).
  \vcite{hedtstueck}{32f} Bei Programmiersprachen handelt es sich
  um \emph{kontextfreie Sprachen}. Diese sind entscheibar
  und somit von einem \gloss{Compiler} verarbeitet werden.
 \vcite{hedtstueck}{16f}
  ...
  \vcite{ghosh}{211} Er ist dafür verantworlich aus dem Ergebnis der
...
\begin{itemize}
 ...
 demnach den von  Dan Roam beschriebenen Prosess des visuellen Denkens
 ...
 umgesetzt ist wächst das Verständis für das, was
 ...
 Außerdem bleibt die Komplexität durch eine höhere Absatrktionsebene beherrschbar. \simplevcite{journals/insk/BrauerCK08}
  ...
  ausdrucksvolle Variablenbezeichnungen und festgeleten Konventionen
  angestrebt.
  ...
  \item \textbf{Skalierbarkeit des Entwicklungsprozesses}\\
...
\begin{itemize}
  ...
  Entwickler der DSL zu tätigen. Ob sich eine Investition lohnt, muss voher
  ...
  wiederverwendet werden, weil dadruch unerwünschte Nebeneffekte
  auftreten können.
...
\vcite{fowler}{28} In einer internen DSL hingenen ist es möglich Methoden
...
ein Computer mit einem Processor und zwei Festplatten
beschrieben.
...
eine eigene Syntax sowie ein eigenes Typsytem besitzt. In der Regel wird mit einer externen DSL ein Skript geschrieben,
welches von einem Programm gelesen wird. Dieser Vorgang wird auch als
parsen bezeichnet. \vcite{fowler}{28} Für den Parser die
...
\gloss{EBNF} beschrieben werden. Zweitens werden bestimmte Aktionen benötigt,
die bei der Bestätigung bestimmter Grammatik-Regeln ausgeführt werden sollen
...
meisten Parser Generatoren mit Code-Generierung, wodruch der Build-Prozess komplexer
wird.
...
Boolean-Wert zurück, der Auskunft darüber gibt, ob die Symbole aus dem DSL-Skript mit den Symbolen übereinstimmen, die laut Grammatik erwartet
werden. \vcite{fowler}{246} Tabelle \ref{tab_RD_Implementierung} enthält die
Implementierungsmöglichkeiten von einfachen Grammatikregeln.
\begin{table}[!htb]
\centering
\small
			\begin{tabular}[c]{|l|l|}
